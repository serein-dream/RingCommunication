...
def adjust_learning_rate(epoch, lr, lr_gamma, lr_step, optimizer, log):
    if epoch in lr_step:
        lr = lr * lr_gamma
...
def run(model, training_loader, test_loader, sampler, optimizer, criterion,
        lr, lr_gamma, lr_step, epochs, log, device):
    rank = dist.get_rank()
    best_train_acc = -1
    best_test_acc = -1
    ...
    for epoch in range(epochs):
        ...
        lr = adjust_learning_rate(epoch, lr, lr_gamma, lr_step, optimizer, log)
